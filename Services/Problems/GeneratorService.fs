...
type GeneratorService(viewFormatter: IViewFormatter,
    ...
    interface IGeneratorService with
        ...
        member this.GenerateProblemSet(problemSet: ProblemSetModel): Async<Result<GeneratedProblemSetId, GenerateFail>> =
            async {
        ...
        member this.GenerateProblem(problem: ProblemModel) =
            async {
                match! this.CreateGeneratedProblem(problem) with
                | Result.Error(fail) -> return Result.Error(fail)
                | Ok(generatedProblem) ->
